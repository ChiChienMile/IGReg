...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # Set visible GPUs *before* importing torch
...
from dataLoder.dataset_1p19q_ratio import Dataset_TrainBal_Ratio as Dataset_self_cls_1
from dataLoder.dataset_1p19q_ratio import Dataset_Test as Dataset_self_cls_test_1

from dataLoder.dataset_idh_ratio import Dataset_TrainBal_Ratio as Dataset_self_cls_2
from dataLoder.dataset_idh_ratio import Dataset_Test as Dataset_self_cls_test_2

from dataLoder.dataset_hl_ratio import Dataset_TrainBal_Ratio as Dataset_self_cls_3
from dataLoder.dataset_hl_ratio import Dataset_Test as Dataset_self_cls_test_3

# Segmentation dataset
from dataLoder.auxiliary_seg_dataset import AuxiliarySegDataset
...
def save_model(model, config):
    ...
    Filename encodes the mean F1 over the three classification tasks,
    the per-task F1 scores, and the global step. Segmentation metrics are
    no longer used for validation or checkpoint naming.
    ...
    name = (
        ...
        "_step_{}.pkl"
    ).format(config['name'], F1, F1_HL, F1_IDH, F1_1p19q, config['global_step'])
...
# ============================================================
# Validation helpers (classification only)
# ============================================================
...
def train(model, init_seg, ratio_list, Cross_Seg=1):
    """
    Main training loop for the three classification tasks. Segmentation data are still used during training if required by the model, but segmentation is not evaluated during validation.
    ...
    Per epoch:
      ...
      4) Validate classification tasks only; log metrics; save the best model by mean F1.
    ...
    train_dataset_seg = AuxiliarySegDataset(split="train", cross=Cross_Seg, transform=val_transforms)
    ...
    init_seg_dataset = AuxiliarySegDataset(split="train", cross=Cross_Seg, transform=val_transforms)
    ...
    # ---------------- Optimizers ----------------
    # Default optimizer is Lion. Set config.optimizer_name = "adam" to use Adam.
    if config.optimizer_name=='adam':
        optim_main = Adam(filter(lambda p: p.requires_grad, model.parameters()),
                          lr=config.lr_self, weight_decay=config.weight_decay)
        optim_g1 = Adam(filter(lambda p: p.requires_grad, model.G1.parameters()),
                        lr=config.lr_self, weight_decay=config.weight_decay)
        optim_g2 = Adam(filter(lambda p: p.requires_grad, model.G2.parameters()),
                        lr=config.lr_self, weight_decay=config.weight_decay)
        optim_g3 = Adam(filter(lambda p: p.requires_grad, model.G3.parameters()),
                        lr=config.lr_self, weight_decay=config.weight_decay)
    else:
    ...
    for epoch in range(config.epochs):
        ...
        # ---------------- Checkpointing ----------------
        # Select by mean F1 across the three classification tasks.
...
if __name__ == '__main__':
    ...
    model = IGReg(n_classes=2, num_channels=32, dim_proj=64, nc_pos=6, nc_neg=6)
