...
KFOLD_NAME_0 = "splits_final_0_1p19q.pkl"
KFOLD_NAME_1 = "splits_final_1_1p19q.pkl"


@dataclass(frozen=True)
class DatasetConfig:
    """
    Configuration for each dataset used in the 1p19q prediction task.
    """
    name: str
    image_root: Path
    split_file_0: Path
...
DATASET_CONFIGS = [
    DatasetConfig(
        ...
        label_getter=get_label_BMIAXNAT_1p19q,
    ),
    DatasetConfig(
        name="MICCAI_2020",
        ...
        label_getter=get_label_201p19q,
    ),
    DatasetConfig(
        name="Public_1p19q",
        image_root=TITLE_DIR / "DATA_ALL/DATA/_1p19q",
        split_file_0=TITLE_DIR / "DATA_ALL/k_fold_Multi_final/_1p19q/splits_final_0.pkl",
        split_file_1=TITLE_DIR / "DATA_ALL/k_fold_Multi_final/_1p19q/splits_final_1.pkl",
        label_file=TITLE_DIR / "DATA_ALL/k_fold_Multi_final/Clinical/labels_1p19q.csv",
        label_getter=get_label_1p19q,
    ),
]
...
def collect_all_records(read_type: str, cross: int) -> List[CaseRecord]:
    """
    Collect all valid 1p19q cases from BMIAXNAT, MICCAI_2020, and Public_1p19q.
    """
    all_records = []
...
class Dataset_TrainBal_Ratio(Dataset):
    """
    Ratio-controlled balanced training dataset for 1p19q prediction.
...
class Dataset_Test(Dataset):
    ...
    Optional modes:
        mask_flag=True, concat=False:
            multiply the input image by the tumor mask.
        mask_flag=True, concat=True:
            concatenate the tumor mask as an additional channel.
        nameflag=True:
            return case name and case path for saving predictions.
    """
