...
KFOLD_NAME_0 = "splits_final_0_HL.pkl"
KFOLD_NAME_1 = "splits_final_1_HL.pkl"


@dataclass(frozen=True)
class DatasetConfig:
    """
    Configuration for each dataset used in the low-/high-grade glioma task.
    """
    name: str
    image_root: Path
    split_file_0: Path
...
DATASET_CONFIGS = [
    DatasetConfig(
        ...
        label_getter=get_label_BMIAXNAT_HL,
    ),
    DatasetConfig(
        name="MICCAI_2020",
...
def collect_all_records(read_type: str, cross: int) -> List[CaseRecord]:
    """
    Collect all valid low-/high-grade glioma cases from BMIAXNAT and MICCAI_2020.
    """
    all_records = []
...
class Dataset_TrainBal_Ratio(Dataset):
    """
    Ratio-controlled balanced training dataset for low-/high-grade glioma prediction.
