...
class VAblock(nn.Module):
    ...
    def __init__(self, num_channels, level, cardinality):
        ...
        self.fusion1 = nn.Sequential(
            nn.Conv3d(num_channels * 4, num_channels, kernel_size=1)
            # nn.Conv3d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU(),
            # nn.Conv3d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU()
        )
        self.fusion2 = nn.Sequential(
            nn.Conv3d(num_channels * 3, num_channels, kernel_size=1)
            # nn.Conv3d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU(),
            # nn.Conv3d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU()
        )
        self.fusion3 = nn.Sequential(
            nn.Conv3d(num_channels * 2, num_channels, kernel_size=1)
            # nn.Conv3d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU(),
            # nn.Conv3d(64, 64, kernel_size=3, padding=1), nn.GroupNorm(32, 64), nn.PReLU()
        )
...
class BiVA(nn.Module):
    ...
    def __init__(self,num_channels, epsilon=1e-4, first_time=False, onnx_export=False, attention=True):
        ...
        self.VAblock1 = VAblock(num_channels, level=1, cardinality=num_channels//2)
        self.VAblock2 = VAblock(num_channels, level=2, cardinality=num_channels//2)
        self.VAblock3 = VAblock(num_channels, level=3, cardinality=num_channels//2)
        self.VAblock4 = VAblock(num_channels, level=4, cardinality=num_channels//2)

        self.pool1 = add_conv3D(num_channels, num_channels, 1, (2, 2, 2))
        self.pool2 = add_conv3D(num_channels, num_channels, 1, (2, 2, 2))
